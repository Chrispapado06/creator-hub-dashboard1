  ...
  return (
    <Screen>
      <Stagger>
        <ScreenHeader
          title="Your dates"
          subtitle="What athletes can find and enquire about."
          action={
            <Button size="sm">
        ...
        <Rise className="space-y-2.5 pt-5">
          ...
          {list.length === 0 && (
            <Card>
              <p className="py-4 text-center text-[13px] text-mist-dim">Nothing here.</p>
            </Card>
          )}
        </Rise>
      </Stagger>
...
function StatusBadge({ status }: { status: OpeningStatus }) {
  ...
  if (status === "full") return <Badge tone="gold">Full</Badge>;
