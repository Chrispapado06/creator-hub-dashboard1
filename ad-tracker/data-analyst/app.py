...
Tabs (one window):
  • Dashboard    — the full tracker dashboard (KPIs, charts, leaderboard).
...
SECRET_KEYS = ["supabase_anon_key", "onlyfans_key", "meta_token", "amplitude_token"]
...
def sync_onlyfans():
    ...
    dashboard reads. Backend (clicks / new fans / revenue) is live;
    ...
    def do_POST(self):
        ...
        if path == "/config":
            ...
            if "selected_accounts" in payload:
                ...
                resync = True
                try:  # keep the creator roster in sync with the picks
...
$("saveCfg").addEventListener("click",async()=>{
  ...
  ["c_supabase_anon_key","c_onlyfans_key","c_meta_token","c_amplitude_key"].forEach(id=>$(id).value="");
  loadConfig();
});
