...
// The cadence owner gets the Morning Card instead (cadence/cadence.mjs, 04:40
// UTC). Two daily pushes to the same person is exactly the failure the card was
// designed against, so he is skipped here — the card carries his tasks AND a
// roll-up of any pipeline steps waiting on him, so nothing is lost. Matched on
// EITHER id, so setting just the channel (the card's own preferred config) is
// enough. Unset both and he simply rejoins the normal digest.
const CADENCE_OWNER_ID = process.env.CADENCE_OWNER_ID;
const CADENCE_CHANNEL_ID = process.env.CADENCE_CHANNEL_ID;
...
const TASKS_QUERY = "standalone_tasks?status=eq.open&select=assignee_id,title,due_date,description,created_at&order=created_at.asc";
...
// One-line gloss of a task's notes, rendered as Discord subtext beneath it.
// Cut at the first sentence where there is one, otherwise on a word boundary —
// a task's notes are usually "what this is and what done looks like", and the
// first sentence carries that. Never mid-word: a truncated instruction reads as
// a different instruction.
function gloss(text, min = 70, max = 165) {
  const s = String(text || "").replace(/\s+/g, " ").trim();
  if (!s) return "";
  if (s.length <= max) return s;

  // Keep whole sentences, and keep taking them until there is enough to be
  // useful. Stopping at the first sentence alone loses the actionable half:
  // "Read spend / CPA / ROAS per creator." drops "Kill or scale one thing."
  let out = "";
  for (const part of s.split(/(?<=[.!?])\s+/)) {
    if (out && (out.length >= min || out.length + part.length > max)) break;
    out = out ? `${out} ${part}` : part;
  }
  if (out && out.length <= max) return out;

  const cut = s.lastIndexOf(" ", max);
  return s.slice(0, cut > min ? cut : max).trim() + "…";
}
...
function buildDigest({ dateLabel, discordUserId, steps, tasks, extra }) {
  ...
  // Notes are dropped wholesale rather than truncated when the message would
  // overrun. postToChannel/editMessage slice at 1900, which would otherwise cut
  // the message off mid-task and silently hide the last items on the list —
  // strictly worse than showing every title with no notes.
  const render = (noteMax) => {
  ...
  if (tasks.length) {
    msg += `\n### 📋 To-do (${tasks.length})\n` +
      tasks.map((t) => {
        const head = `- ${t.title}${t.due_date ? ` · _due ${prettyDate(t.due_date)}_` : ""}`;
        const note = noteMax ? gloss(t.description, Math.min(70, noteMax - 20), noteMax) : "";
        return note ? `${head}\n  -# ${note}` : head;
      }).join("\n") + "\n";
  }
  if (content.length) {
  ...
  // Degrade in steps rather than all-or-nothing: a long list keeps SHORT notes
  // instead of losing every note the moment one person's day gets busy.
  for (const noteMax of [165, 90, 0]) {
    const msg = render(noteMax);
    if (msg.length <= 1880 || noteMax === 0) return msg;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const c of chatters) {
      ...
      if ((CADENCE_OWNER_ID && c.discord_user_id === CADENCE_OWNER_ID) ||
          (CADENCE_CHANNEL_ID && c.discord_channel_id === CADENCE_CHANNEL_ID)) { skipped++; continue; } // → Morning Card
