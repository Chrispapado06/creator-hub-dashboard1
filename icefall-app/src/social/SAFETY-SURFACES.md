...
| 14 | **Search result (person)** | `screens/Search.tsx:632-651` — `HitRow` | no | no | **Route, not a control.** A hit's `to` is `/explore/people/:id`, which is row #7. A menu on a search row is a thumb hazard on a list somebody is scrolling fast. The one thing to check: `SearchHit.subtitle` is the person's own bio (`search/people.ts`), so an abusive bio is readable without opening the profile — acceptable only because the profile is one tap away and will have the control |
...
| 16 | **Leaderboard entry** | `screens/explore/Leaderboard.tsx:432-466` — the row `Link` | no | no | Route to #7 (`to={/explore/people/${entry.athleteId}}`, `:434`). No control on the row: it is a ranked list, and a menu on it invites tapping the wrong person's name |
