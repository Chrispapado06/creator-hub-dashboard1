...
/**
 * A LAUREL AWARD, built to the design the owner sent on 2026-09-06.
 *
 * The reference is a shadcn component (`Awards`, variant `"award"`) and this is
 * that design, not that file. Why it was rebuilt rather than pasted in:
 *
 *   · IT IS WRITTEN AGAINST TOKENS THIS APP DOES NOT HAVE. `bg-card`,
 *     `text-primary`, `text-muted-foreground`, `border` — shadcn's palette.
 *     ICEFALL has its own (`snow`, `mist`, `graphite`, `azure`) and no shadcn
 *     theme layer, so those class names compile to nothing here. The laurel
 *     path, the proportions, the type scale and the layout are all the
 *     reference's; only the colours are ours.
 *   · ITS LEVEL COLOURS INCLUDE GOLD, and gold is spoken for. `index.css` is
 *     explicit that `--ice-gilt` means "somebody is selling you this" — it is
 *     the colour of a paid placement — so a gold award would read as sponsored.
 *     The top tier is `--ice-credential` instead, the colour this app already
 *     uses for a qualification it has checked, which is much closer to what an
 *     award is.
 *   · The five other variants (stamp, certificate, badge, sticker, id-card) are
 *     not here. Nothing asked for them, and an unused variant is a design
 *     decision nobody has made yet.
 */

/**
 * The tier. Bronze/silver/platinum keep the reference's names; gold is the one
 * that changed, for the reason above.
 */
export type AwardLevel = "bronze" | "silver" | "gold" | "platinum";

const LEVEL_STYLE: Record<AwardLevel, { chip: string; ink: string }> = {
  bronze: { chip: "linear-gradient(90deg, #A9713B, #7C4E24)", ink: "#FFFFFF" },
  silver: { chip: "linear-gradient(90deg, #C9CFDA, #8B94A6)", ink: "#1D222A" },
  /* NOT `--ice-gilt`. See the header. */
  gold: {
    chip: "linear-gradient(90deg, var(--ice-credential), #A98536)",
    ink: "#1D222A",
  },
  platinum: { chip: "linear-gradient(90deg, #E7ECF4, #A7B0C0)", ink: "#1D222A" },
};

/** The reference's own laurel, unchanged — its path data, at its viewBox. */
function Laurel({ className }: { className?: string }) {
  return (
    <svg
      aria-hidden
      viewBox="0 0 892.77 688.08"
      className={cn("absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-snow/85", className)}
      fill="currentColor"
    >
...
/**
 * How big the composition is drawn. THE DESIGN IS IDENTICAL AT BOTH SIZES —
 * wreath, tier chip, title, rule, subtitle, recipient, year, in that order.
 *
 * The owner asked for the award beside their username and got a redrawn badge
 * back — a bare wreath with the year in it — and said so: "not the way you
 * added it, the prev one just smaller. you changed the design." Correct. A
 * "compact version" that drops four of the seven elements is a different mark
 * wearing the same laurel, and shrinking is not licence to redesign.
 *
 * SO `small` IS A TRANSFORM, NOT A SECOND SET OF SIZES. The composition is
 * drawn once, at its own dimensions, and `scale()` shrinks the whole thing —
 * every proportion, every gap, the weight of the rule against the type. A
 * hand-tuned small variant drifts from the original the moment either is
 * touched; a scale factor cannot. It is literally the previous design, smaller,
 * which is what was asked for.
 *
 * The wrapper carries the SCALED box so the layout around it is correct —
 * `transform` does not change how much room an element takes, so without it the
 * award would reserve its full height and push the profile's stats down a
 * hundred pixels of empty space.
 */
export type AwardSize = "full" | "small";

/** The composition's own dimensions, before any scaling. */
const NATURAL_W = 300;
const NATURAL_H = 214;

/**
 * Badge scale, set on the owner's eye across two passes — 0.42, then 0.32, were
 * both still "too big" beside a name. At 0.26 the badge stands about as tall as
 * the name it sits next to, which is where it stopped.
 *
 * THE WORDS ARE NOT READABLE AT THIS SIZE and are not meant to be: the badge
 * says "this person has an award", and `AwardBadge` below is what says which
 * one. That is why the small size is a button rather than a picture.
 */
const SMALL_SCALE = 0.26;

export interface AwardWreathProps {
  /** The big word. "WINNER" in the reference. */
  title: string;
  /** What it was for. */
  subtitle: string;
  /** Who holds it. Italic, under the rule, as in the reference. */
  recipient?: string;
  /** When. */
  date?: string;
  level?: AwardLevel;
...
export function AwardWreath({
  ...
  const composition = (
    ...
    >
      {/* The wreath sits behind everything and takes the ink colour, so it
          works on both themes without a second asset. */}
      <Laurel className="h-[190px] w-[92%]" />

      <div className="relative z-10 px-10 text-center">
        <span
          className="mb-2 inline-block rounded-md px-3.5 py-1 text-[11px] font-semibold tracking-[0.14em]"
          style={{ background: style.chip, color: style.ink }}
        >
          {level.toUpperCase()}
        </span>

        <h3 className="text-[30px] font-black leading-none tracking-tight text-snow">{title}</h3>

        <div className="mx-auto my-2.5 h-px w-32 bg-hairline-strong" />

        <p className="mx-auto max-w-[13rem] text-[13.5px] font-light leading-snug text-snow">
          {subtitle}
        </p>

        {recipient !== undefined && <p className="mt-2 text-[12px] italic text-mist">{recipient}</p>}

        {date !== undefined && <p className="mt-0.5 text-[14px] font-bold text-snow">{date}</p>}
      </div>
    </div>
  );

  if (size === "full") return <div className={className}>{composition}</div>;

  return (
    <div
      className={cn("overflow-hidden", className)}
      style={{ width: NATURAL_W * SMALL_SCALE, height: NATURAL_H * SMALL_SCALE }}
      /* The whole award is one image at this size — the words are texture
         rather than something anybody reads — so it is announced instead. */
      role="img"
      aria-label={`${title}: ${subtitle}${recipient ? `, ${recipient}` : ""}${date ? `, ${date}` : ""}`}
    >
      <div style={{ transform: `scale(${SMALL_SCALE})`, transformOrigin: "top left" }}>
        {composition}
      </div>
    </div>
  );
...
/* -------------------------------------------------------------------------- */
/* The badge, and what opens behind it                                         */
/* -------------------------------------------------------------------------- */

/**
 * THE AWARD BESIDE A NAME, AND THE FULL THING WHEN TAPPED.
 *
 * The owner, 2026-09-06: "if you can keep same design, and when people click on
 * it they can see what it was for. so shows winner, gold, and year".
 *
 * So the badge is the same composition at `SMALL_SCALE` — nothing redrawn —
 * inside a button, and the dialog it opens is the same composition again at
 * full size, where the tier, the title, what it was for, whose it is and the
 * year are all legible. One design, two sizes, and the small one exists to be
 * pressed.
 *
 * IT IS A REAL BUTTON. A scaled-down award that only carried a `title`
 * attribute would be a tooltip on a phone, which is nothing at all — there is
 * no hover. It is keyboard-reachable and announces the award in full through
 * `aria-label` on the wrapper inside `AwardWreath`.
 */
export function AwardBadge({
  className,
  ...award
}: AwardWreathProps & { className?: string }) {
  ...
  return (
    <>
      <button
        ...
        aria-label={`Award: ${award.title}, ${award.subtitle}${award.date ? `, ${award.date}` : ""}. Open`}
        className={cn(
          "shrink-0 rounded-lg transition-transform active:scale-95",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-azure/60",
          className,
        )}
      >
        <AwardWreath {...award} size="small" />
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center px-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-[340px] rounded-[24px] border border-hairline-strong bg-graphite pb-5 pt-2 shadow-[var(--ice-shadow-pop)]"
          >
            <AwardWreath {...award} size="full" className="mx-auto" />

            <button
              type="button"
              aria-label="Close"
              onClick={() => setOpen(false)}
              className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full border border-hairline-strong bg-obsidian/70 text-mist transition-colors hover:text-snow"
            >
