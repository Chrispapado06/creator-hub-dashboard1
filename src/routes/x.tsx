...
import { Users, TrendingUp, Edit2, Check, X as XIcon } from "lucide-react";
...
function XPage() {
  ...
  const load = async () => {
    ...
    const [{ data: cs }, { data: sas }] = await Promise.all([
      ...
      supabase.from("social_accounts").select("*").eq("platform", "x"),
    ]);
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight">X</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          Track X accounts and follower growth per creator. Update stats manually.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            <SiX className="h-4 w-4 text-foreground" /> Connected
          </div>
