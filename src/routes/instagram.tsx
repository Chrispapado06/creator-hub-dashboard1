...
function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      {/* Brand hero — gradient panel with the headline numbers. */}
      <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-card via-card/80 to-[#E1306C]/10 p-6">
        <div aria-hidden className="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-[#E1306C]/15 blur-3xl pointer-events-none" />
        <div aria-hidden className="absolute -bottom-20 -left-20 h-64 w-64 rounded-full bg-[#F77737]/10 blur-3xl pointer-events-none" />
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground font-semibold">
              <SiInstagram className="h-3.5 w-3.5" style={{ color: IG_PINK }} />
              Instagram presence
            </div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              {posts.length.toLocaleString()} posts tracked · {posts30d} in last 30d · {totalLikes.toLocaleString()} total likes
            </div>
          </div>
      ...
      {topPost && (
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <span className="h-1 w-1 rounded-full bg-[#E1306C]" />
...
function ModernStat({
  ...
  const toneCls = {
    pink:    { chip: "bg-[#E1306C]/15 text-[#E1306C]",  value: "" },
    cyan:    { chip: "bg-cyan-500/15 text-cyan-400",    value: "" },
...
function AnalyticsTab({
  ...
  // Per-account Infloww attribution. Same shape as TikTok's
  // analytics revenue strip — totals + per-account breakdown.
  ...
  return (
    <div className="space-y-6">
      {/* Revenue from Instagram — total platform income across every
          account this creator runs, with per-account breakdown below. */}
