...
function FacebookPage() {
  ...
  const load = async () => {
    ...
    const [{ data: cs }, { data: sas }] = await Promise.all([
      ...
      supabase.from("social_accounts").select("*").eq("platform", "facebook"),
    ]);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Track Facebook pages and follower growth per creator.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="text-xs text-muted-foreground mt-1">across all connected pages</div>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            <SiFacebook className="h-4 w-4 text-[#1877F2]" /> Connected
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="text-xs text-muted-foreground mt-1">per page</div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="text-left font-medium px-4 py-3">Page Name</th>
                <th className="text-right font-medium px-4 py-3">Followers</th>
                <th className="text-right font-medium px-4 py-3">Page Likes</th>
                    ...
                    {isEditing ? (
                      <>
                        <td className="px-4 py-2">
                          <Input className="h-7 text-xs w-36" placeholder="Page name" value={editState.username} onChange={(e) => setEditState({ ...editState, username: e.target.value })} />
                        </td>
                        <td className="px-4 py-2 text-right">
                          <Input className="h-7 text-xs w-24 text-right ml-auto" type="number" placeholder="0" value={editState.followers_count} onChange={(e) => setEditState({ ...editState, followers_count: e.target.value })} />
                        </td>
                        <td className="px-4 py-2 text-right">
                          <Input className="h-7 text-xs w-24 text-right ml-auto" type="number" placeholder="0" value={editState.following_count} onChange={(e) => setEditState({ ...editState, following_count: e.target.value })} />
                        </td>
                        <td className="px-4 py-2 text-right">
                          <Input className="h-7 text-xs w-20 text-right ml-auto" type="number" placeholder="0" value={editState.posts_count} onChange={(e) => setEditState({ ...editState, posts_count: e.target.value })} />
                        </td>
                        <td className="px-4 py-2">
                          <Input className="h-7 text-xs w-36" placeholder="Notes" value={editState.notes} onChange={(e) => setEditState({ ...editState, notes: e.target.value })} />
                        </td>
                        <td className="px-4 py-2" />
                        <td className="px-4 py-2 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => saveEdit(c.id)} className="rounded p-1 hover:bg-success/20 text-success transition-colors">
                              <Check className="h-4 w-4" />
                            </button>
                            <button onClick={() => setEditing(null)} className="rounded p-1 hover:bg-secondary text-muted-foreground transition-colors">
                              <X className="h-4 w-4" />
                            </button>
                          </div>
                        </td>
                      ...
                      <>
                        <td className="px-4 py-3 text-muted-foreground">
                          {acct?.username ?? <span className="text-muted-foreground/40 italic">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <button onClick={() => startEdit(c.id)} className="rounded p-1.5 text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors">
      ...
      <p className="text-xs text-muted-foreground">
        Stats are updated manually. Click the edit icon on any row to update follower counts and page name.
      </p>
    </div>
  );
}
