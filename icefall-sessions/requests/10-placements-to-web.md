# Request 09 → Session 02 (web): read placements from the live database
...
the live database, and icefall-web renders operators from fixtures — so a
placement made in the CRM never reaches a visitor. The ask: read the mountain
...
- `catalogue_head()` / `catalogue_since(ts)` — anon-callable functions for a
  ...
  This is the decision-20 "scheduled for next update on the web" mechanism.
